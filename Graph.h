...
#include<functional>		//Anpassung wegen S533. Sonst lässt sich kein lambda(mit [n]) an Function übergeben
...
//	typedef double Fct(double);							//Anpassung wegen S533. Sonst lässt sich kein lambda(mit [n]) an Function übergeben
	...
	public:
		...
		*/
		...
		void clear_points() { points.clear(); }// von mir hinzugefügt
	private:
		vector<Point> points;	// not used by all shapes
		Color lcolor{ FL_BLACK };								//Farbe Schwarz als Standard um nicht ständig anpassen zu müssen
		Line_style ls{ 0 };
